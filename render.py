...
def render_svg_bars(hours, current_hour, reference_price=None, reference_label=""):
    ...
    # Extra room to the right so the reference line/label can extend past the bars
    right_margin = 90
    width = bars_w + right_margin
    ...
    baseline = f'<line x1="0" y1="{chart_h}" x2="{bars_w}" y2="{chart_h}" stroke="#111" stroke-width="2"></line>'
    ...
    # On-chart min/max labels, near the top-left and bottom-left of the plotted range
    y_max_price = y_of(pmax)
    y_min_price = y_of(pmin)
    minmax_labels = (
        f'<text x="2" y="{y_max_price - 5}" font-size="11" fill="#888">{pmax:.1f} ct</text>'
        f'<text x="2" y="{y_min_price + 13}" font-size="11" fill="#888">{pmin:.1f} ct</text>'
    )
    ...
    return (
        ...
        f'with OPTIMA Aktiv reference line and day min/max">'
        + "".join(bars_svg) + baseline + reference_line + minmax_labels + labels + "</svg>"
    )
...
def render_html(today_hours, week_hours, now):
    ...
    if week_hours:
        ...
        avg_line = f"Today's avg {today_avg:.1f} ct \u2014 {abs(pct):.0f}% {direction} 7-day average"
    else:
        avg_line = f"Today's avg {today_avg:.1f} ct"
    ...
    css = (
        ...
        ".top{border-bottom:2px solid #111;padding-bottom:10px;margin-bottom:10px;font-size:4vw;}"
        ".optima{font-size:2.6vw;color:#555;margin-bottom:14px;}"
